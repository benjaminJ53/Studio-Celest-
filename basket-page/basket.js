function renderBasket() {
  ...
  basket.forEach(item => {
    ...
    <div><h4>${item.name}</h4><p>Price: £${item.price.toFixed(2)}</p><p>Qty: ${item.quantity}</p><p>Line Total: £${item.lineTotal.toFixed(2)}</p></div>
  ...
  let displayTotal = total > 500 ? total * 0.9 : total;

  totalEl.textContent = `£${displayTotal.toFixed(2)}${total > 500 ? " (10% discount)" : ""}`;
}

function removeFromBasket(productId) {
  let basket = getBasket().filter(item => item.id !== productId);
  saveBasket(basket);
  renderBasket();
  renderBasketCount();
}
